...
for (col in columns) {
  ...
  # Plot the data
  df %>% 
    ggplot(aes_string(col, "Perceived_Stress")) + 
    geom_point() +
    geom_smooth(method = "lm", se = FALSE, color = "red", size = 1.5) + 
    ggtitle(paste(col, "and Perceived Stress")) +
    labs(x = col, y = "Perceived Stress") +
    theme_minimal() +
  ...
  ggsave(paste0("plots/Stress_", col, ".png"), width = 8, height = 6)
}
```
### 2.2 Big5 and Grades
...
```{r}
ggplot(df_long, aes(value, Perceived_Stress)) +
  geom_smooth(aes(group = variable, color=variable), method = "lm", se = FALSE) +
  scale_color_brewer(palette = "Dark2") +
  theme_minimal() + 
  ylim(2.5,4) +
  ggtitle("Correlation between Perceived Stress Level and Personality Trades") +
  labs(
    subtitle = "Perceived Stress Level and Personality Trades extracted fromt the Big 5 questionnaire",
    x = "Trade Score", 
...
# save the plot
ggsave("plots/Combined_Stress_Big5.png", width = 8, height = 6)
```
...
```{r}
cor.test(df$GPA, df$Conscientiousness, method = "pearson")
```
